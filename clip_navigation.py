...
class TurtlebotImageCapture:
    ...
    def rotate_robot(self, target_angle_deg):
        ...
        vel_msg = Twist()
        vel_msg.angular.z = 0.5  # Adjust angular speed as needed

        while not rospy.is_shutdown():
            current_yaw = self.get_yaw_from_odom()
            if abs(target_yaw - current_yaw) < 0.05:  # Yaw threshold
                break
    ...
    def execute_and_compare(self, command):
        ...
        # Log captured angles
        rospy.loginfo(f"Captured angles: {captured_angles}")

        # Rotate to best angle
        if best_angle is not None:
            rospy.loginfo(f"Rotating to best angle: {best_angle} degrees")
            self.rotate_robot(best_angle - current_angle)  # Adjust to rotate from the current angle

        # Move forward towards the target
        move_distance = 1.0  # Set the distance to move towards the target
